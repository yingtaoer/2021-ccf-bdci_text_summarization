...
# #                 model.save_weights('./results/ch_t5_base_best_model.weights')  # 保存模型
#             metrics['best_bleu'] = self.best_bleu
#             print('valid_data:', metrics)
        
    def on_epoch_end(self,epoch,logs=None):
        rouge_l = self.evaluate(valid_data)
...
def do_train(model):
    data = load_data('train_dataset.csv')
    data = data[:200]
# train_data, val_data = train_test_split(train, shuffle=True, random_state=0, test_size=0.1)
...
#     model.load_weights('./results/ch_t5_base_best_model.weights')
